...
static const char *pvar_class_names[] = {"state",          "level",         "size",    "percentage",
                                         "high watermark", "low watermark", "counter", "aggregate",
                                         "timer",          "generic"};
...
int mca_base_pvar_find(const char *project, const char *framework, const char *component,
                       const char *name)
{
    char *full_name;
    int ret, index;
...
static int mca_base_pvar_default_get_value(const mca_base_pvar_t *pvar, void *value,
                                           void *obj_handle)
{
    /* not used */
    (void) obj_handle;
...
static int mca_base_pvar_default_set_value(mca_base_pvar_t *pvar, const void *value,
                                           void *obj_handle)
{
    /* not used */
    (void) obj_handle;
...
static int mca_base_pvar_notify_ignore(mca_base_pvar_t *pvar, mca_base_pvar_event_t event,
...
int mca_base_pvar_register(const char *project, const char *framework, const char *component,
                           const char *name, const char *description,
                           mca_base_var_info_lvl_t verbosity, int var_class,
                           ...
                           mca_base_set_value_fn_t set_value, mca_base_notify_fn_t notify,
                           void *ctx)
{
    ...
    case MCA_BASE_PVAR_CLASS_TIMER:
        if (MCA_BASE_VAR_TYPE_UNSIGNED_INT != type && MCA_BASE_VAR_TYPE_UNSIGNED_LONG != type
            && MCA_BASE_VAR_TYPE_UNSIGNED_LONG_LONG != type && MCA_BASE_VAR_TYPE_DOUBLE != type) {
            return OPAL_ERR_BAD_PARAM;
        }
        break;
...
int mca_base_component_pvar_register(const mca_base_component_t *component, const char *name,
                                     ...
                                     int var_class, mca_base_var_type_t type,
                                     ...
                                     void *ctx)
{
    ...
    return mca_base_pvar_register(component->mca_project_name, component->mca_type_name,
                                  ...
                                  var_class, type, enumerator, bind, flags | MCA_BASE_PVAR_FLAG_IWG,
                                  get_value, set_value, notify, ctx);
}
...
int mca_base_pvar_update_all_handles(int index, const void *obj)
{
    ...
    OPAL_LIST_FOREACH_SAFE (handle, next, &pvar->bound_handles, mca_base_pvar_handle_t) {
        handle = (mca_base_pvar_handle_t *) ((char *) handle
                                             - offsetof(mca_base_pvar_handle_t, list2));
        ...
        if (mca_base_pvar_is_sum(pvar) || mca_base_pvar_is_watermark(pvar)) {
            ...
            if (mca_base_pvar_is_continuous(pvar)) {
                if (mca_base_pvar_is_sum(pvar)) {
                    ...
                    ret = pvar->get_value(pvar, pvar_handle->current_value,
                                          pvar_handle->obj_handle);
                }
...
int mca_base_pvar_handle_update(mca_base_pvar_handle_t *handle)
{
            ...
            for (i = 0; i < handle->count; ++i) {
                ...
                case MCA_BASE_VAR_TYPE_UNSIGNED_LONG_LONG:
                    ((unsigned long long *) handle->current_value)[i]
                        += ((unsigned long long *) handle->tmp_value)[i]
                ...
                if (MCA_BASE_PVAR_CLASS_LOWWATERMARK == handle->pvar->var_class) {
                    ...
                    case MCA_BASE_VAR_TYPE_UNSIGNED_INT:
                        ((unsigned *) handle->current_value)[i]
                            = opal_min(((unsigned *) handle->tmp_value)[i],
                                       ((unsigned *) handle->current_value)[i]);
                        break;
                    case MCA_BASE_VAR_TYPE_UNSIGNED_LONG:
                        ((unsigned long *) handle->current_value)[i]
                            = opal_min(((unsigned long *) handle->tmp_value)[i],
                                       ((unsigned long *) handle->current_value)[i]);
                        break;
                    case MCA_BASE_VAR_TYPE_UNSIGNED_LONG_LONG:
                        ((unsigned long long *) handle->current_value)[i]
                            = opal_min(((unsigned long long *) handle->tmp_value)[i],
                                       ((unsigned long long *) handle->current_value)[i]);
                        break;
                    case MCA_BASE_VAR_TYPE_DOUBLE:
                        ((double *) handle->current_value)[i]
                            = opal_min(((double *) handle->tmp_value)[i],
                                       ((double *) handle->current_value)[i]);
                        break;
                    default:
                        /* shouldn't happen */
                    ...
                    case MCA_BASE_VAR_TYPE_UNSIGNED_INT:
                        ((unsigned *) handle->current_value)[i]
                            = opal_max(((unsigned *) handle->tmp_value)[i],
                                       ((unsigned *) handle->current_value)[i]);
                        break;
                    case MCA_BASE_VAR_TYPE_UNSIGNED_LONG:
                        ((unsigned long *) handle->current_value)[i]
                            = opal_max(((unsigned long *) handle->tmp_value)[i],
                                       ((unsigned long *) handle->current_value)[i]);
                        break;
                    case MCA_BASE_VAR_TYPE_UNSIGNED_LONG_LONG:
                        ((unsigned long long *) handle->current_value)[i]
                            = opal_max(((unsigned long long *) handle->tmp_value)[i],
                                       ((unsigned long long *) handle->current_value)[i]);
                        break;
                    case MCA_BASE_VAR_TYPE_DOUBLE:
                        ((double *) handle->current_value)[i]
                            = opal_max(((double *) handle->tmp_value)[i],
                                       ((double *) handle->current_value)[i]);
                        break;
                    default:
                        /* shouldn't happen */
    ...
    /* XXX -- TODO -- For watermarks this function will have to be invoked for each handle whenever
     * the underlying value is updated. */
...
int mca_base_pvar_handle_read_value(mca_base_pvar_handle_t *handle, void *value)
{
    ...
    if (mca_base_pvar_is_sum(handle->pvar) || mca_base_pvar_is_watermark(handle->pvar)
        ...
        memmove(value, handle->current_value,
                handle->count * ompi_var_type_sizes[handle->pvar->type]);
    } else {
...
int mca_base_pvar_handle_start(mca_base_pvar_handle_t *handle)
{
    ...
    if ((handle->pvar->flags & MCA_BASE_PVAR_FLAG_CONTINUOUS) || handle->started) {
    ...
    if (mca_base_pvar_is_sum(handle->pvar)) {
        ...
        /* Find the current watermark. is this correct in the case where a watermark is started,
           stopped, then restarted? Probably will need to add a check. */
        ret = handle->pvar->get_value(handle->pvar, handle->current_value, handle->obj_handle);
        if (OPAL_SUCCESS != ret) {
            return ret;
        }
...
int mca_base_pvar_dump(int index, char ***out, mca_base_var_dump_type_t output_type)
{
    ...
    if (MCA_BASE_VAR_DUMP_PARSABLE == output_type) {
        ...
        *out = (char **) calloc(line_count + 1, sizeof(char *));
        ...
        (void) opal_asprintf(out[0] + line++, "%sread-only:%s", tmp,
                             mca_base_pvar_is_readonly(pvar) ? "true" : "false");
        (void) opal_asprintf(out[0] + line++, "%scontinuous:%s", tmp,
                             mca_base_pvar_is_continuous(pvar) ? "true" : "false");
        (void) opal_asprintf(out[0] + line++, "%satomic:%s", tmp,
                             mca_base_pvar_is_atomic(pvar) ? "true" : "false");
        ...
        if (NULL != pvar->enumerator) {
            for (i = 0; i < enum_count; ++i) {
                ...
                ret = pvar->enumerator->get_value(pvar->enumerator, i, &enum_value, &enum_string);
                if (OPAL_SUCCESS != ret) {
                    continue;
                }

                (void) opal_asprintf(out[0] + line++, "%senumerator:value:%d:%s", tmp, enum_value,
                                     enum_string);
            }
        }
...
OBJ_CLASS_INSTANCE(mca_base_pvar_t, opal_object_t, mca_base_pvar_contructor,
