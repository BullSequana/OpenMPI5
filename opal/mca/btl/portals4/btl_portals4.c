...
static int btl_portals4_init_interface(void)
{
    ...
    for (interface = 0; interface < mca_btl_portals4_component.num_btls; interface++) {
        ...
        /* create event queue */
        ret = PtlEQAlloc(portals4_btl->portals_ni_h, mca_btl_portals4_component.recv_queue_size,
                         &portals4_btl->recv_eq_h);
        if (PTL_OK != ret) {
            opal_output_verbose(1, opal_btl_base_framework.framework_output,
                                "%s:%d: PtlEQAlloc failed for NI %d: %d", __FILE__, __LINE__,
                                interface, ret);
            goto error;
        }
        ...
        ret = PtlPTAlloc(portals4_btl->portals_ni_h, PTL_PT_ONLY_TRUNCATE, portals4_btl->recv_eq_h,
                         REQ_BTL_TABLE_ID, &portals4_btl->recv_idx);
        if (PTL_OK != ret) {
            opal_output_verbose(1, opal_btl_base_framework.framework_output,
                                "%s:%d: PtlPTAlloc failed for NI %d: %d", __FILE__, __LINE__,
                                interface, ret);
            goto error;
        }
        OPAL_OUTPUT_VERBOSE((90, opal_btl_base_framework.framework_output,
                             "PtlPTAlloc (recv_idx) OK for NI %d recv_idx=%d", interface,
                             portals4_btl->recv_idx));

        if (portals4_btl->recv_idx != REQ_BTL_TABLE_ID) {
            opal_output_verbose(1, opal_btl_base_framework.framework_output,
                                "%s:%d: PtlPTAlloc did not allocate the requested PT: %d", __FILE__,
                                __LINE__, portals4_btl->recv_idx);
            goto error;
        }
        ...
        ret = PtlMDBind(portals4_btl->portals_ni_h, &md, &portals4_btl->zero_md_h);
        if (PTL_OK != ret) {
            opal_output_verbose(1, opal_btl_base_framework.framework_output,
                                "%s:%d: PtlMDBind failed for NI %d: %d", __FILE__, __LINE__,
                                interface, ret);
            goto error;
        }
        OPAL_OUTPUT_VERBOSE((90, opal_btl_base_framework.framework_output,
                             "PtlMDBind (zero-length md=%d) OK for NI %d", portals4_btl->zero_md_h,
                             interface));
        ...
        ret = PtlMDBind(portals4_btl->portals_ni_h, &md, &portals4_btl->send_md_h);
        if (PTL_OK != ret) {
            opal_output_verbose(1, opal_btl_base_framework.framework_output,
        ...
        me.min_free = 0;
        me.uid = PTL_UID_ANY;
        me.options = PTL_ME_OP_PUT | PTL_ME_EVENT_LINK_DISABLE | PTL_ME_EVENT_COMM_DISABLE
        ...
        me.ignore_bits = BTL_PORTALS4_CONTEXT_MASK | BTL_PORTALS4_SOURCE_MASK
                         | BTL_PORTALS4_TAG_MASK;
        ret = PtlMEAppend(portals4_btl->portals_ni_h, portals4_btl->recv_idx, &me,
                          PTL_OVERFLOW_LIST, NULL, &portals4_btl->long_overflow_me_h);
        if (PTL_OK != ret) {
            opal_output_verbose(1, opal_btl_base_framework.framework_output,
                                "%s:%d: PtlMEAppend failed for NI %d: %d", __FILE__, __LINE__,
                                interface, ret);
            goto error;
        }
        OPAL_OUTPUT_VERBOSE((90, opal_btl_base_framework.framework_output,
                             "PtlMEAppend (overflow list) OK for NI %d", interface));
...
error:
    opal_output_verbose(1, opal_btl_base_framework.framework_output,
                        "Error in btl_portals4_init_interface");
...
static int create_endpoint(int interface, opal_proc_t *proc, mca_btl_base_endpoint_t **endpoint)
{
    int ret;
    size_t size;
    ptl_process_t *id;

    OPAL_MODEX_RECV(ret, &mca_btl_portals4_component.super.btl_version, &proc->proc_name,
                    (void **) &id, &size);
    ...
    OPAL_OUTPUT_VERBOSE((90, opal_btl_base_framework.framework_output,
                         "btl/portals4: %d NI(s) declared in the modex",
                         (int) (size / sizeof(ptl_process_t))));
...
static int create_peer_and_endpoint(int interface, opal_proc_t *proc, ptl_process_t *phys_peer,
                                    mca_btl_base_endpoint_t **endpoint)
{
    int ret;
    size_t size;
    ...
    opal_output_verbose(50, opal_btl_base_framework.framework_output,
                        ...
                        phys_peer->phys.pid, phys_peer->phys.nid);

    return OPAL_SUCCESS;
}

static int create_maptable(struct mca_btl_portals4_module_t *portals4_btl, size_t nprocs,
                           opal_proc_t **procs, mca_btl_base_endpoint_t **endpoint)
{
    int ret;
    ...
    for (uint32_t i = 0; i < nprocs; i++) {
        ...
        if (opal_proc_local_get()->proc_arch != curr_proc->proc_arch) {
            ...
            opal_output_verbose(1, opal_btl_base_framework.framework_output,
                                ...
                                OPAL_NAME_PRINT(curr_proc->proc_name), curr_proc->proc_arch,
                                opal_proc_local_get()->proc_arch);
            return OPAL_ERR_NOT_SUPPORTED;
        }

        ret = create_peer_and_endpoint(portals4_btl->interface_num, curr_proc, &maptable[i],
                                       &endpoint[i]);
        if (OPAL_SUCCESS != ret) {
            opal_output_verbose(1, opal_btl_base_framework.framework_output,
                               ...
                               opal_bitmap_t *reachable)
{
    ...
    if (mca_btl_portals4_component.need_init && portals4_btl->portals_num_procs > 0) {
        if (mca_btl_portals4_component.use_logical) {
            ...
            if (OPAL_SUCCESS != ret) {
                opal_output_verbose(
                    1, opal_btl_base_framework.framework_output,
        ...
        if (OPAL_SUCCESS != ret) {
            opal_output_verbose(1, opal_btl_base_framework.framework_output,
                                "%s:%d: portals4 interface initialization failed: %d", __FILE__,
                                __LINE__, ret);
            return ret;
        }
...
int mca_btl_portals4_del_procs(struct mca_btl_base_module_t *btl, size_t nprocs,
                               struct opal_proc_t **procs,
                               struct mca_btl_base_endpoint_t **btl_peer_data)
{
                                                  ...
                                                  uint8_t order, size_t size, uint32_t flags)
{
    ...
    if (size <= portals4_btl->super.btl_eager_limit) {
        ...
        if (NULL == frag)
            return NULL;
        frag->segments[0].base.seg_len = size <= portals4_btl->super.btl_max_send_size
                                             ? size
...
mca_btl_base_registration_handle_t *mca_btl_portals4_register_mem(mca_btl_base_module_t *btl_base,
                                                                  mca_btl_base_endpoint_t *endpoint,
                                                                  void *base, size_t size,
                                                                  uint32_t flags)
{
    ...
    handle = (mca_btl_base_registration_handle_t *) malloc(
    ...
    me.min_free = 0;
    me.uid = PTL_UID_ANY;
    me.options = PTL_ME_OP_GET | PTL_ME_EVENT_LINK_DISABLE | PTL_ME_EVENT_COMM_DISABLE
                 | PTL_ME_EVENT_UNLINK_DISABLE;
    ...
    me.ignore_bits = BTL_PORTALS4_PROTOCOL_MASK | BTL_PORTALS4_CONTEXT_MASK
                     | BTL_PORTALS4_SOURCE_MASK;
    me.ignore_bits = 0;

    ret = PtlMEAppend(portals4_btl->portals_ni_h, portals4_btl->recv_idx, &me, PTL_PRIORITY_LIST,
                      handle, &(handle->me_h));
    if (PTL_OK != ret) {
        opal_output_verbose(1, opal_btl_base_framework.framework_output,
    ...
    OPAL_OUTPUT_VERBOSE((90, opal_btl_base_framework.framework_output,
                         "PtlMEAppend (mca_btl_portals4_register_mem) handle=%p, me_h=%d start=%p "
                         "length=%ld rank=%x nid=%x pid=%x match_bits=%lx\n",
                         (void *) handle, handle->me_h, me.start, me.length, me.match_id.rank,
...
int mca_btl_portals4_finalize(struct mca_btl_base_module_t *btl)
{
    struct mca_btl_portals4_module_t *portals4_btl = (struct mca_btl_portals4_module_t *) btl;
...
void mca_btl_portals4_free_module(mca_btl_portals4_module_t *portals4_btl)
{
    ...
    while (portals4_btl->portals_outstanding_ops > 0) {
        ...
        OPAL_OUTPUT_VERBOSE((90, opal_btl_base_framework.framework_output,
                             "Call to mca_btl_portals4_component_progress (3)\n"));
    ...
    if (!PtlHandleIsEqual(PTL_EQ_NONE, portals4_btl->recv_eq_h)) {
        ...
        if (PTL_OK != ret)
            OPAL_OUTPUT_VERBOSE(
                (90, opal_btl_base_framework.framework_output, "Error freeing EQ recv: %d", ret));
        OPAL_OUTPUT_VERBOSE((90, opal_btl_base_framework.framework_output,
                             "PtlEQFree: recv_eq_h=%d portals4_btl=%p", portals4_btl->recv_eq_h,
                             (void *) portals4_btl));
    ...
    if (!PtlHandleIsEqual(portals4_btl->portals_ni_h, PTL_INVALID_HANDLE)) {
        ...
        if (PTL_OK != ret)
            OPAL_OUTPUT_VERBOSE((90, opal_btl_base_framework.framework_output,
                                 "Error returned by PtlNIFini: %d\n", ret));
        OPAL_OUTPUT_VERBOSE((90, opal_btl_base_framework.framework_output,
                             "PtlNIFini: portals_ni_h=%d portals4_btl=%p",
    ...
    if (PTL_OK != ret)
        OPAL_OUTPUT_VERBOSE(
            (90, opal_btl_base_framework.framework_output, "Error freeing recv list: %d", ret));
}
