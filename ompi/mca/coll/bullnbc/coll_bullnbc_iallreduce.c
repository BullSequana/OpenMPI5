...
static inline int allred_sched_diss(int rank, int p, int count, MPI_Datatype datatype, ptrdiff_t gap, const void *sendbuf,
                                    void *recvbuf, MPI_Op op, char inplace, BULLNBC_Schedule *schedule, void **tmpbuf);
static inline int allred_sched_recursivedoubling(int rank, int p, const void *sendbuf, void *recvbuf,
                                                 ...
                                                 char inplace, BULLNBC_Schedule *schedule, void **tmpbuf);
static inline int allred_sched_ring(int r, int p, int count, MPI_Datatype datatype, const void *sendbuf,
                                    void *recvbuf, MPI_Op op, ptrdiff_t ext, BULLNBC_Schedule *schedule,
                                    void **tmpbuf);
static inline int allred_sched_linear(int rank, int rsize, const void *sendbuf, void *recvbuf, int count,
                                      MPI_Datatype datatype, ptrdiff_t gap, MPI_Op op, ptrdiff_t ext,
                                      BULLNBC_Schedule *schedule, void **tmpbuf);
static inline int allred_sched_redscat_allgather(
    ...
    BULLNBC_Schedule *schedule, void **tmpbuf, const struct ompi_communicator_t *comm);
...
int ompi_coll_bullnbc_allreduce_check_forced_init (coll_bullnbc_force_algorithm_mca_param_indices_t *mca_param_indices)
{
  ...
  (void) mca_base_var_enum_create("coll_bullnbc_allreduce_algorithms", allreduce_algorithms, &new_enum);
  (void) mca_base_component_var_register(&mca_coll_bullnbc_component.super.collm_version,
  ...
  mca_base_component_var_register(&mca_coll_bullnbc_component.super.collm_version,
                                  "iallreduce_algorithm_segmentsize",
                                  "Segment size in bytes used by default for iallreduce algorithms. Only has meaning if algorithm is forced and supports segmenting. 0 bytes means no segmentation.",
                              ...
                              struct mca_coll_base_module_2_4_0_t *module, bool persistent)
{
  ...
  if(inplace || (!mca_coll_bullnbc_component.reduce_allow_non_commutative_support && !ompi_op_is_commute(op))) {
    alg = NBC_ARED_BINOMIAL;
    goto selected_rule;
  }
  if(mca_coll_bullnbc_component.use_dynamic_rules) {
    if(0 != mca_coll_bullnbc_component.forced_params[ALLREDUCE].algorithm) {
      /* if op is not commutative or MPI_IN_PLACE was specified we have to deal with it */
      alg = mca_coll_bullnbc_component.forced_params[ALLREDUCE].algorithm - 1; /* -1 is to shift from algorithm ID to enum */
      goto selected_rule;
    }
    if(bullnbc_module->com_rules[ALLREDUCE]) {
      int algorithm,dummy1,dummy2,dummy3;
      algorithm = ompi_coll_base_get_target_method_params (bullnbc_module->com_rules[ALLREDUCE],
  ...
  if (alg == NBC_ARED_REDSCAT_ALLGATHER && !(count >= nprocs_pof2 && ompi_op_is_commute(op))){
    alg = NBC_ARED_RING;
  }
selected_rule:
  opal_output_verbose(10, mca_coll_bullnbc_component.stream,
                      "Bullnbc iallreduce : algorithm %d (no segmentation supported)",
                      alg + 1);
    ...
    if (p == 1) {
      ...
      switch(alg) {
        ...
        case NBC_ARED_REDSCAT_ALLGATHER:
          ...
          break;
        case NBC_ARED_RING:
          res = allred_sched_ring(rank, p, count, datatype, sendbuf, recvbuf, op, ext, schedule, &tmpbuf);
          break;
        case NBC_ARED_RDBL:
...
static inline int allred_sched_diss( // NOSONAR cognitive complexity
                                    int rank, int p, int count, MPI_Datatype datatype, ptrdiff_t gap, const void *sendbuf, void *recvbuf,
                                    MPI_Op op, char inplace, BULLNBC_Schedule *schedule, void **sched_tmpbuf) {
  int root, vrank, maxr, vpeer, peer, res;
  char *rbuf, *lbuf, *buf;
  char tmprbuf, tmplbuf;

  root = 0; /* this makes the code for ireduce and iallreduce nearly identical - could be changed to improve performance */
  RANK2VRANK(rank, vrank, root);
  maxr = ceil_of_log2(p);

  int firstred = 1; /* data in sendbuf, 0 means data in lbuf */

  /* Need to end initialisation with buffers lbuf and rbuf on host (for OP)
   * and with data on host (in sendbuf or lbuf) */
  void * true_recvbuf = recvbuf;
  char tmprecvbuf = false;
  int sent_first = vrank % 2;
  void *tmpbuf = *sched_tmpbuf;

  char rbuf_is_cuda = opal_iallreduce_use_device_pointers;
  uint64_t flags;
  int dev_id;
  rbuf_is_cuda = rbuf_is_cuda || opal_accelerator.check_addr(recvbuf, &dev_id, &flags);

  if (GPU_LIKELYHOOD(rbuf_is_cuda)) {
      /* recvbuf cannot be used for OP, need a new tmp buffer */
      ptrdiff_t span = opal_datatype_span(&datatype->super, count, &gap);
      tmpbuf = realloc(tmpbuf, 2*span); //NOSONAR size>0 => span>0
      *sched_tmpbuf = tmpbuf;

      recvbuf = (void*) (span - gap);
      tmprecvbuf = true;
  }
  ...
  if (0 == (maxr%2)) {
    ...
    tmplbuf = tmprecvbuf;
    if (inplace) {
        /* Use lbuf instead of sendbuf to get data at the first time */
        firstred = 0;
        if (GPU_LIKELYHOOD(rbuf_is_cuda && !sent_first)) {
            /* Need data on host */
            res = NBC_Sched_copy(true_recvbuf, false, count, datatype,
                                 lbuf, tmplbuf, count, datatype,
                                 schedule, true);
    ...
    tmprbuf = tmprecvbuf;
    if (inplace) {
        /* Data can only be placed in lbuf or sendbuf */
        res = NBC_Sched_copy(true_recvbuf, false, count, datatype,
                             lbuf, tmplbuf, count, datatype,
                             schedule, true);
        if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
  ...
  if (GPU_LIKELYHOOD(!sent_first /* PML is able to send from cuda buffer */
                     && !inplace
                     && (opal_iallreduce_use_device_pointers
  ...
  for (int r = 1; r <= maxr ; ++r) {
    if ((vrank % (1 << r)) == 0) {
      ...
      if (peer < p) {
        ...
        /* recvbuf can also be in tmpbuf (for example if real buffer is GPU) */
        char tmp = tmprbuf; tmprbuf = tmplbuf; tmplbuf = tmp;
      }
    } else {
  ...
  if (vrank != 0) {
    for (int r = 0; r < maxr ; ++r) {
      if ((vrank >= (1 << r)) && (vrank < (1 << (r + 1)))) {
        ...
        /* The rank may help for broadcasting data */
        recvbuf = true_recvbuf;
        tmprecvbuf = false;
        res = NBC_Sched_recv(recvbuf, tmprecvbuf, count, datatype, peer, schedule, false);
        if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
            return res;
        }
      }
  ...
  else if (GPU_LIKELYHOOD(rbuf_is_cuda)) { /* vrank == 0 */
      /* First sched on sendbuf is OP, that needs host buffer */
      res = NBC_Sched_copy(recvbuf, tmprecvbuf, count, datatype,
                           true_recvbuf, false, count, datatype,
                           schedule, false);
  }
  ...
  for (int r = 0; r < maxr; ++r) {
    if (((vrank + (1 << r) < p) && (vrank < (1 << r))) || (vrank == 0)) {
      VRANK2RANK(peer, vrank + (1 << r), root);
      res = NBC_Sched_send (recvbuf, tmprecvbuf, count, datatype, peer, schedule, false);
      if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
        return res;
      }
    }
 ...
 */
static inline int allred_sched_redscat_allgather(   // NOSONAR cognitive complexity
    int rank, int comm_size, int count, MPI_Datatype datatype, ptrdiff_t gap,
    const void *sbuf, void *rbuf, MPI_Op op, char inplace,
    BULLNBC_Schedule *schedule, void **sched_tmpbuf, const struct ompi_communicator_t *comm)
{
    ...
    void * tmpbuf = *sched_tmpbuf;
    void * true_rbuf = rbuf;
    int rbuf_is_cuda = opal_iallreduce_use_device_pointers;
    uint64_t flags;
    int dev_id;
    rbuf_is_cuda = rbuf_is_cuda || opal_accelerator.check_addr(rbuf, &dev_id, &flags);
    if (GPU_LIKELYHOOD(rbuf_is_cuda)) {
        /* rbuf cannot be used for OP, need a new tmp buffer */
        ptrdiff_t span = opal_datatype_span(&datatype->super, count, &gap);
        tmpbuf = realloc(tmpbuf, 2*span); //NOSONAR size>0 => span>0
        *sched_tmpbuf = tmpbuf;

        rbuf = (void*) ((char*) tmpbuf) + span - gap;
        if (inplace) {
            res = NBC_Sched_copy((char *)true_rbuf, false, count, datatype,
                                 rbuf, false, count, datatype, schedule, true);
            if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) { goto cleanup_and_return; }
        }
    }
    /* If not in place, sbuf is copied anyway */
     ...
     if (vrank != -1) {
        ...
        step = nsteps - 1;
         for (int mask = nprocs_pof2 >> 1; mask > 0; mask >>= 1) {
             ...
             */
            res = NBC_Sched_send((char *)rbuf + (ptrdiff_t)rindex[step] * extent, // NOSONAR /* S3519: Sonar thinks that rindex[step] may be an invalid read */
    ...
    if (rank < 2 * nprocs_rem) {
        if (rank % 2 != 0) {
            ...
            if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) { goto cleanup_and_return; }
            if (GPU_LIKELYHOOD(rbuf_is_cuda)) {
                /* Add a barrrier on comms before copying */
                res = NBC_Sched_barrier (schedule);
                if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) { goto cleanup_and_return; }
            }
        } else {
    ...
    if (GPU_LIKELYHOOD(rbuf_is_cuda)) {
        ...
        res = NBC_Sched_copy(rbuf, false, count, datatype,
                             true_rbuf, false, count, datatype,
                             schedule, false);
    }
 ...
 *
 */
static inline int allred_sched_recursivedoubling(   //NOSONAR cognitive complexity
                                                 int rank, int p, const void *sendbuf, void *recvbuf,
                                                 int count, MPI_Datatype datatype, ptrdiff_t gap, MPI_Op op,
                                                 char inplace, BULLNBC_Schedule *schedule, void **sched_tmpbuf)
{
  int res, pof2, nprocs_rem, vrank;
  char *tmpsend, *tmprecv, *tmpswap = NULL;
  void * tmpbuf = *sched_tmpbuf;

  tmprecv = (char*) recvbuf;
  char recvbuf_is_cuda =  opal_iallreduce_use_device_pointers;
  uint64_t flags;
  int dev_id;
  recvbuf_is_cuda = recvbuf_is_cuda  || opal_accelerator.check_addr(recvbuf, &dev_id, &flags);
  if (GPU_LIKELYHOOD(recvbuf_is_cuda)) {
      /* recvbuf cannot be used for OP, need a new tmp buffer */
      ptrdiff_t span = opal_datatype_span(&datatype->super, count, &gap);
      tmpbuf = realloc(tmpbuf, 2*span); //NOSONAR size>0 => span>0
      *sched_tmpbuf = tmpbuf;
      tmprecv = ((char*) tmpbuf) + span - gap;
  }
  ...
  */
  ...
  if (tmpsend != recvbuf
      /* All computation is done on CPU, result must be copied on GPU anyway */
      || GPU_LIKELYHOOD(recvbuf_is_cuda)
     ) {
...
static inline int allred_sched_ring (int r, int p, int count, MPI_Datatype datatype, const void *sendbuf, void *recvbuf, MPI_Op op,
                                     ptrdiff_t ext, BULLNBC_Schedule *schedule, void **sched_tmpbuf) {
  ...
  void * tmpbuf = *sched_tmpbuf;
  /* No need to take care of GPU "in place" case because this algorithm is
   * not supposed being called in place (even in full CPU mode) */
  void * true_recvbuf = recvbuf;
  char sendbuf_is_cuda = opal_iallreduce_use_device_pointers;
  char recvbuf_is_cuda = opal_iallreduce_use_device_pointers;
  uint64_t flags;
  int dev_id;
  sendbuf_is_cuda = sendbuf_is_cuda  || opal_accelerator.check_addr(sendbuf, &dev_id, &flags);
  recvbuf_is_cuda = recvbuf_is_cuda  || opal_accelerator.check_addr(recvbuf, &dev_id, &flags);
  if (GPU_LIKELYHOOD(recvbuf_is_cuda)) {
      /* recvbuf cannot be used for OP, need a new tmp buffer */
      ptrdiff_t gap;
      ptrdiff_t span = opal_datatype_span(&datatype->super, count, &gap);
      if(sendbuf_is_cuda){
          /* tmobuf is already used by cuda sendbuf buffering */
          tmpbuf = realloc(tmpbuf, 2*span); //NOSONAR size>0 => span>0
          *sched_tmpbuf = tmpbuf;

          recvbuf = (void*) ((char*) tmpbuf) + span - gap;
      } else { /* only recvbuf is cuda buffer */
          recvbuf = (void*) ((char*) tmpbuf) - gap;
      }
      /* this algotithm does not support inplace sendbuf */
  }

  if (GPU_LIKELYHOOD(sendbuf_is_cuda)) {
      /* sendbuf cannot be used for OP, need a new tmp buffer */
      ptrdiff_t gap;
      opal_datatype_span(&datatype->super, count, &gap);
      const void * true_sendbuf = sendbuf;
      sendbuf = (void*) ((char*) tmpbuf) - gap;

      /* this algotithm does not support inplace sendbuf */
      res = NBC_Sched_copy((char *)true_sendbuf, false, count, datatype,
                           (char*) sendbuf, false, count, datatype, schedule, true);
      if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) { goto cleanup_and_return; }
  }
...
static inline int allred_sched_linear(int rank, int rsize, const void *sendbuf, void *recvbuf, int count, MPI_Datatype datatype,
				      ptrdiff_t gap, MPI_Op op, ptrdiff_t ext, BULLNBC_Schedule *schedule, void **sched_tmpbuf) {
  int res;
  ...
  void * tmpbuf = *sched_tmpbuf;
  void * true_recvbuf = recvbuf;
  int root_rbuf_is_cuda = false;
  uint64_t flags;
  int dev_id;
  if (0 == rank && GPU_LIKELYHOOD(opal_iallreduce_use_device_pointers
                    || opal_accelerator.check_addr(recvbuf, &dev_id, &flags))) {
      /* Expensive buffer type query only if root since it is the only rank to use tmp buffers */
      root_rbuf_is_cuda = true;
  ...
  if (0 == rank) {
    ...
    if (GPU_LIKELYHOOD(root_rbuf_is_cuda)) {
        /* Copy reduced data on original Cuda buffer */
        res = NBC_Sched_copy(recvbuf, false, count, datatype,
                             true_recvbuf, false, count, datatype,
                             schedule,false);
        if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) { return res; }
