...
static int
ompi_coll_base_neighbor_alltoallv_cart(const void *sbuf, const int scounts[], const int sdisps[], // NOSONAR complexity
...
static int
ompi_coll_base_neighbor_alltoallv_graph(const void *sbuf, const int scounts[], const int sdisps[],
                                        struct ompi_datatype_t *sdtype, void *rbuf, const int rcounts[],
                                        const int rdisps[], struct ompi_datatype_t *rdtype,
                                        struct ompi_communicator_t *comm, mca_coll_base_module_t *module)
{
...
static int
ompi_coll_base_neighbor_alltoallv_dist_graph(const void *sbuf, const int scounts[], const int sdisps[],
                                             struct ompi_datatype_t *sdtype, void *rbuf, const int rcounts[],
                                             const int rdisps[], struct ompi_datatype_t *rdtype,
                                             struct ompi_communicator_t *comm, mca_coll_base_module_t *module)
{
...
static int
ompi_coll_base_controlled_neighbor_alltoallv_dist_graph(const void *sbuf, const int scounts[], const int sdisps[],
                                                        struct ompi_datatype_t *sdtype, void *rbuf, const int rcounts[],
                                                        const int rdisps[], struct ompi_datatype_t *rdtype,
                                                        ...
                                                        int limit)//NOSONAR
{
    ...
    /* post all receives first */
    for (neighbor = 0; neighbor < indegree ; ++neighbor) {
        err = MCA_PML_CALL(irecv((char *) rbuf + rdisps[neighbor] * rdextent, rcounts[neighbor], rdtype,
    ...
    for (neighbor = 0 ; neighbor < outdegree ; ++neighbor) {
        if (neighbor < limit) {
            err = MCA_PML_CALL(isend((char *) sbuf + sdisps[neighbor] * sdextent, scounts[neighbor], sdtype,
                               outedges[neighbor], MCA_COLL_BASE_TAG_NEIGHBOR_BASE, MCA_PML_BASE_SEND_STANDARD,
...
static int
ompi_coll_base_controlled_neighbor_alltoallv_cart(const void *sbuf, const int scounts[], const int sdisps[], // NOSONAR complexity
                                                  struct ompi_datatype_t *sdtype, void *rbuf, const int rcounts[],
                                                  const int rdisps[], struct ompi_datatype_t *rdtype,
                                                  struct ompi_communicator_t *comm, const mca_coll_base_module_t *module,
                                                  int limit)//NOSONAR
{
    ...
    for (dim = 0, peer = 0; dim < cart->ndims ; ++dim, peer += 2) {
        ...
        if (MPI_PROC_NULL != srank) {
            err = MCA_PML_CALL(irecv((char *) rbuf + rdisps[peer] * rdextent, rcounts[peer], rdtype, srank,
                                     MCA_COLL_BASE_TAG_NEIGHBOR_BASE - 2 * dim, comm, &recv_rq[nb_rrq]));
            nb_rrq++;
            if (OMPI_SUCCESS != err) goto end_err;
        }
        if (MPI_PROC_NULL != drank) {
            err = MCA_PML_CALL(irecv((char *) rbuf + rdisps[peer+1] * rdextent, rcounts[peer+1], rdtype, drank,
    ...
    for (dim = 0, peer = 0 ; dim < cart->ndims ; ++dim, peer += 2) {
        ...
        if (MPI_PROC_NULL != srank) {
            if (nb_srq < limit) {
                err = MCA_PML_CALL(isend((char *) sbuf + sdisps[peer] * sdextent, scounts[peer], sdtype, srank,
        ...
        if (MPI_PROC_NULL != drank) {
            if (nb_srq < limit) {
                err = MCA_PML_CALL(isend((char *) sbuf + sdisps[peer+1] * sdextent, scounts[peer+1], sdtype, drank,
...
int ompi_coll_base_neighbor_alltoallv(const void *sbuf, const int scounts[], const int sdisps[],
                                      struct ompi_datatype_t *sdtype, void *rbuf, const int rcounts[],
                                      const int rdisps[], struct ompi_datatype_t *rdtype,
                                      struct ompi_communicator_t *comm, mca_coll_base_module_t *module)
{
    ...
    if (OMPI_COMM_IS_CART(comm)) {
        return ompi_coll_base_neighbor_alltoallv_cart(sbuf, scounts, sdisps, sdtype, rbuf,
                                                      rcounts, rdisps, rdtype, comm, module);
    } else if (OMPI_COMM_IS_GRAPH(comm)) {
        return ompi_coll_base_neighbor_alltoallv_graph(sbuf, scounts, sdisps, sdtype, rbuf,
                                                       rcounts, rdisps, rdtype, comm, module);
    } else if (OMPI_COMM_IS_DIST_GRAPH(comm)) {
        return ompi_coll_base_neighbor_alltoallv_dist_graph(sbuf, scounts, sdisps, sdtype, rbuf,
                                                            rcounts, rdisps, rdtype, comm, module);
    }

    return OMPI_ERR_NOT_SUPPORTED;
}
int ompi_coll_base_neighbor_alltoallv_controlled(const void *sbuf, const int scounts[], const int sdisps[],
                                                 struct ompi_datatype_t *sdtype, void *rbuf, const int rcounts[],
                                                 const int rdisps[], struct ompi_datatype_t *rdtype,
                                                 struct ompi_communicator_t *comm, mca_coll_base_module_t *module,
                                                 int limit)
{
    ...
    if (OMPI_COMM_IS_DIST_GRAPH(comm)) {
        return ompi_coll_base_controlled_neighbor_alltoallv_dist_graph(sbuf, scounts, sdisps, sdtype, rbuf,
                                                                       rcounts, rdisps, rdtype, comm, module,
                                                                       limit);
    } else if (OMPI_COMM_IS_CART(comm)) {
        return ompi_coll_base_controlled_neighbor_alltoallv_cart(sbuf, scounts, sdisps, sdtype, rbuf,
                                                                 rcounts, rdisps, rdtype, comm, module,
                                                                 limit);
    }
