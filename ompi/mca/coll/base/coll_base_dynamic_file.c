...
int ompi_coll_base_read_rules_config_file (char *fname, int format_version, ompi_coll_base_alg_rule_t** rules, int n_collectives)
{
    ...
    switch (format_version) {
        ...
        case NODES_COMM_MSG_FORMAT:
            ...
            break;
        case NODES_COMM_MSG_MREQ_FORMAT:
            opal_output_verbose(10,ompi_coll_base_framework.framework_output,"Reading dynamic rule format %d : <nodes_nb, comm_size, msg_size> with max requests\n", NODES_COMM_MSG_FORMAT);
            break;
        default:
    ...
    for (x=0;x<X;x++) { /* for each collective */
        ...
        if (NODES_COMM_MSG_FORMAT == format_version
            || NODES_COMM_MSG_MREQ_FORMAT == format_version) {
        ...
        for (node_rule = 0 ; node_rule < nodes_rules_number ; node_rule++) { /* for each nodes number */
            ...
            for (ncs=0;ncs<NCS;ncs++) {	/* for each comm size */
                ...
                for (nms=0;nms<NMS;nms++) {	/* for each msg size */
                    ...
                    if (NODES_COMM_MSG_MREQ_FORMAT == format_version) {
                        if( (getnext (fptr, &maxr) < 0) || (maxr < 0) ) {
                            opal_output_verbose(1,ompi_coll_base_framework.framework_output,"Could not read target max_requests for collective ID %ld com rule %d msg rule %d at around line %d\n", CI, ncs, nms, fileline);
                            ret = -7;
                            goto on_file_error;
                        }
                    } else {
                    ...
                    msg_p->result_max_requests = maxr;
