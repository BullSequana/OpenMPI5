...
int   ompi_coll_tuned_dynamic_rules_fileformat = 0;
...
bool  ompi_coll_tuned_heterogenous_ddt_support = true;
...
static int tuned_register(void)
{
    ...
    ompi_coll_tuned_dynamic_rules_fileformat = 0;
    (void) mca_base_component_var_register(&mca_coll_tuned_component.super.collm_version,
    ...
    (void) mca_base_component_var_register(&mca_coll_tuned_component.super.collm_version, "verbose",
                                           "Verbose level of the tuned coll component."
                                           " Examples: 0: no verbose, 1: errors, 10: basic, 50: dynamic rules, 100: maximum verbosity level.",
                                           MCA_BASE_VAR_TYPE_INT, NULL, 0, 0,
                                           OPAL_INFO_LVL_9,
                                           MCA_BASE_VAR_SCOPE_READONLY,
    ...
    (void) mca_base_component_var_register(&mca_coll_tuned_component.super.collm_version,
                                           "datatype_support",
                                           "Switch used to decide if derived datatype with different sizes are supported in default communications pattern."
                                           " Are concerned : gather_linear_sync",
                                           MCA_BASE_VAR_TYPE_BOOL, NULL, 0, 0,
                                           OPAL_INFO_LVL_2,
                                           MCA_BASE_VAR_SCOPE_READONLY,
                                           &ompi_coll_tuned_heterogenous_ddt_support);
    ...
    ompi_coll_tuned_neighbor_alltoallv_intra_check_forced_init(&ompi_coll_tuned_forced_params[NEIGHBOR_ALLTOALLV]);
    ompi_coll_tuned_neighbor_alltoallw_intra_check_forced_init(&ompi_coll_tuned_forced_params[NEIGHBOR_ALLTOALLW]);
...
static int tuned_open(void)
{
    ...
    opal_output_verbose(10, ompi_coll_tuned_stream, "coll:tuned:component_open: done!");
...
static int tuned_close(void)
{
    ...
    /* close stream */
    if(ompi_coll_tuned_stream >= 0) {
        opal_output_close(ompi_coll_tuned_stream);
    }
