...
#pragma weak MPIX_Win_allocate_notify = PMPIX_Win_allocate_notify
#endif
#define MPIX_Win_allocate_notify PMPIX_Win_allocate_notify

static const char FUNC_NAME[] = "MPIX_Win_allocate_notify";

int MPIX_Win_allocate_notify(MPI_Aint size, int disp_unit, MPI_Info info,
                             MPI_Comm comm, void *baseptr, MPI_Win *win)
{
    ...
    ret = ompi_win_allocate_notify((size_t)size, disp_unit, &(info->super),
                                   comm, baseptr, win);
    if (OMPI_SUCCESS != ret) {
        *win = MPI_WIN_NULL;
        return OMPI_ERRHANDLER_INVOKE(comm, MPI_ERR_WIN, FUNC_NAME);
