...
#pragma weak MPIX_Pbcastr_init = PMPIX_Pbcastr_init
#endif
#define MPIX_Pbcastr_init PMPIX_Pbcastr_init
#endif

static const char FUNC_NAME[] = "MPIX_Pbcastr_init";


int MPIX_Pbcastr_init(void *buffer, size_t parts, size_t count, MPI_Datatype datatype, MPI_Request * reqs,
                    int root, MPI_Comm comm, MPI_Info info, MPI_Request *request)
{
    ...
    if (OPAL_UNLIKELY(NULL == comm->c_coll->coll_pbcastr_init)) {
        return OMPI_ERRHANDLER_INVOKE(comm, MPI_ERR_UNSUPPORTED_OPERATION, FUNC_NAME);
    }

    err = comm->c_coll->coll_pbcastr_init(buffer, parts, count, datatype, reqs, root, comm,
                                        info, request,
                                        comm->c_coll->coll_pbcastr_init_module);
