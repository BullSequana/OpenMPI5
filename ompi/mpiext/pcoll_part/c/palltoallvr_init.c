...
#        pragma weak MPIX_Palltoallvr_init = PMPIX_Palltoallvr_init
#    endif
#    define MPIX_Palltoallvr_init PMPIX_Palltoallvr_init
#endif

static const char FUNC_NAME[] = "MPIX_Palltoallvr_init";

int MPIX_Palltoallvr_init(const void *sendbuf,
                          ...
                          MPI_Request *request)
{
    ...
    if (OPAL_UNLIKELY(NULL == comm->c_coll->coll_palltoallvr_init)) {
        return OMPI_ERRHANDLER_INVOKE(comm, MPI_ERR_UNSUPPORTED_OPERATION, FUNC_NAME);
    }
    /* Invoke the coll component to perform the back-end operation */
    err = comm->c_coll->coll_palltoallvr_init(sendbuf, sendpartitions, sendcounts, sdispls, sendtype, srequests,
                                              recvbuf, recvpartitions, recvcounts, rdispls, recvtype, rrequests,
                                              comm, info, request,
                                              comm->c_coll->coll_palltoallvr_init_module);
