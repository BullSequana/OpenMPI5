...
#pragma weak MPIX_Preduce_init = PMPIX_Preduce_init
#endif
#define MPIX_Preduce_init PMPIX_Preduce_init
#endif

static const char FUNC_NAME[] = "MPIX_Preduce_init";


int MPIX_Preduce_init(const void *sendbuf, void *recvbuf, size_t parts, int count,
                       ...
                       MPI_Info info, MPI_Request *request)
{
    ...
    if (OPAL_UNLIKELY(NULL == comm->c_coll->coll_preduce_init)) {
        return OMPI_ERRHANDLER_INVOKE(comm, MPI_ERR_UNSUPPORTED_OPERATION, FUNC_NAME);
    }
    err = comm->c_coll->coll_preduce_init(sendbuf, recvbuf, parts, count, datatype,
                                           op, root, comm, info, request,
                                           comm->c_coll->coll_preduce_init_module);
