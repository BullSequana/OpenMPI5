...
#pragma weak MPIX_Palltoallr_init = PMPIX_Palltoallr_init
#endif
#define MPIX_Palltoallr_init PMPIX_Palltoallr_init
#endif

static const char FUNC_NAME[] = "MPIX_Palltoallr_init";


int MPIX_Palltoallr_init(const void *sendbuf, int sendparts, int sendcount, MPI_Datatype sendtype,  MPI_Request *srequests,
                       void *recvbuf, int recvparts, int recvcount, MPI_Datatype recvtype, MPI_Request *rrequests,
                       MPI_Comm comm, MPI_Info info, MPI_Request *request)
{
    ...
    MEMCHECKER(
        ...
        /* TODO Check requests arrays memory */
    ...
    if (OPAL_UNLIKELY(NULL == comm->c_coll->coll_palltoallr_init)) {
        return OMPI_ERRHANDLER_INVOKE(comm, MPI_ERR_UNSUPPORTED_OPERATION, FUNC_NAME);
    }

    /* Invoke the coll component to perform the back-end operation */
    err = comm->c_coll->coll_palltoallr_init(sendbuf, sendparts, sendcount, sendtype, srequests,
                                           recvbuf, recvparts, recvcount, recvtype, rrequests, comm, info,
                                           request, comm->c_coll->coll_palltoallr_init_module);
