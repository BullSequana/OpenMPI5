...
#pragma weak MPIX_Pgatherr_init = PMPIX_Pgatherr_init
#endif
#define MPIX_Pgatherr_init PMPIX_Pgatherr_init
#endif

static const char FUNC_NAME[] = "MPIX_Pgatherr_init";


int MPIX_Pgatherr_init(const void *sendbuf, int sendparts, int sendcount,
                       ...
                       int root, MPI_Comm comm, MPI_Info info, MPI_Request *request)
{
    ...
    if (OPAL_UNLIKELY(NULL == comm->c_coll->coll_pgatherr_init)) {
        return OMPI_ERRHANDLER_INVOKE(comm, MPI_ERR_UNSUPPORTED_OPERATION, FUNC_NAME);
    }

    err = comm->c_coll->coll_pgatherr_init(sendbuf, sendparts, sendcount, sendtype, sendreqs,
                                           recvbuf, recvparts, recvcount, recvtype, recvreqs,
                                           root, comm, info, request,
                                           comm->c_coll->coll_pgatherr_init_module);
